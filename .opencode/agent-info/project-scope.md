...
src/
  ...
  ui/
    ...
    statePanel.js    → Sidebar inputs (laws, mats, cores, settings)
    optimizerPanel.js → Best next + full path table with farming details
    afkPanel.js      → Single AFK farming recommendation
    wikiPanel.js     → Reference tables (marks, tiers, laws)
    markTimePanel.js → Mark time estimator with milestone sim
    styles.css       → Dark theme (~630 lines)
```

Game data lives in `data/*.json`. Adding entries = no code changes (except new metric types → update `ALL_METRICS` in `optimizer.js`).
