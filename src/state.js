...
export function createState(initialLaws = {}, initialMaterials = {}, initialCores = 0, reincarnation = false, materialRateMult = 1, coreRateMult = 1, ashSecret = false, stage300 = false, ashTree = false, divinityMat = false, divinityCore = false) {
  const lawLevels = {};
...
export function computeMultipliers(state) {
  ...
  if (state.ashTree) { matMult *= 2; }
  if (state.divinityMat) { matMult *= 2; }
  if (state.divinityCore) { coreMult *= 2; }
